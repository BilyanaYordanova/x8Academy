...
class StudentsWithHighestAndLowestMarkTest {

	StudentsWithHighestAndLowestMark object = new StudentsWithHighestAndLowestMark();
	...
	@Test
	void test1_StudentsWithHighestMark() {
		fillList();
		List<Student> listStudentsWithHighestMark = object.studentsWithHighestMark(students);
		assertTrue(listStudentsWithHighestMark.contains(IvanPetrov));
		assertTrue(listStudentsWithHighestMark.contains(MariyaGeorgieva));
		assertFalse(listStudentsWithHighestMark.contains(MarioGeorgiev));
	}

	@Test
	void test2_StudentsWithLowestMark() {
		fillList();
		List<Student> listStudentsWithLowestMark = object.studentsWithLowestMark(students);
		assertTrue(listStudentsWithLowestMark.contains(AntonIliev));
		assertTrue(listStudentsWithLowestMark.contains(IvaIvanova));
		assertFalse(listStudentsWithLowestMark.contains(MarioGeorgiev));
	}
}
